#!/usr/bin/env python3
"""
Train một adapter ViCoQA trong process sạch (thoát Jupyter zombie VRAM).
Usage:
    ...
    try:
        ...
        def _safe_warmup(model, expanded_device_map, hf_quantizer=None, **kwargs):
            if torch.cuda.is_available():
                ...
                print(f"[V5] caching_allocator_warmup SKIPPED (free={free / 1024**3:.2f} GiB)", flush=True)
            return None
...
def preflight(min_free_gib=MIN_FREE_VRAM_GIB):
    ...
    if free_gib < min_free_gib:
        raise SystemExit(
            ...
            "Không thể load Qwen2.5-1.5B. Reset GPU trước (không phải lỗi code).\n"
        )
