...
ADAPTER_SUBMISSION_DIRS = {
    "lora": "qwen2.5-3b-instruct-lora-vicoqa",
    "tinylora": "qwen2.5-3b-instruct-tinylora-vicoqa",
    "dora": "qwen2.5-3b-instruct-dora-vicoqa",
    "delora": "qwen2.5-3b-instruct-delora-vicoqa",
}
...
def _validate_adapter_files(adapter_path):
    ...
    candidates.extend(sorted(adapter_path.glob("adapter_model*.safetensors")))
    candidates.extend(sorted(adapter_path.glob("*.safetensors")))
    st_path = next((p for p in candidates if p.exists() and p.stat().st_size > 1024), None)
    if st_path is None:
        raise FileNotFoundError(f"Thiếu weight file trong {adapter_path}")
...
def eval_one_adapter(method_name, adapter_path, dialogs, max_seq_length):
    ...
    pred_file = f"eval_preds_{method_name}_{EVAL_SPLIT}.json"
    with open(pred_file, "w", encoding="utf-8") as f:
        json.dump(preds, f, ensure_ascii=False)
    print(f"Saved predictions → {pred_file} ({len(preds)} turns)", flush=True)
...
if RUN_METRIC_EVAL:
    ...
    line = "=" * 72
    print("\n" + line)
    print(f"  SO SÁNH ADAPTERS — ViCoQA ({EVAL_SPLIT}) | Qwen2.5-3B-Instruct")
    print(line)
    ...
    save_payload = {
        ...
        "pred_files": {k: v["metrics"].get("pred_file") for k, v in all_results.items()},
    ...
    for method, result in all_results.items():
        print(f"  - {result['metrics'].get('pred_file')}")
else:
    print("RUN_METRIC_EVAL=False — bỏ qua metric eval.")

# Submission export chỉ cần khi EVAL_SPLIT != test (tránh infer test 2 lần)
if RUN_SUBMISSION_EXPORT and EVAL_SPLIT != "test":
...
elif RUN_SUBMISSION_EXPORT and EVAL_SPLIT == "test":
    print("EVAL_SPLIT=test — đã có eval_preds_*_test.json; bỏ qua export submission trùng.")
else:
    print("RUN_SUBMISSION_EXPORT=False — bỏ qua results.json.")
'''

cells = [
    md("""# ViCoQA — Qwen2.5-3B: train 4 adapters (Unsloth)

Train **tuần tự** với **cùng data / prompt / seed / epochs**:

1. **LoRA** → `qwen2.5-3b-instruct-lora-vicoqa`
2. **TinyLoRA** → `qwen2.5-3b-instruct-tinylora-vicoqa`
3. **DoRA** → `qwen2.5-3b-instruct-dora-vicoqa`
4. **DeLoRA** → `qwen2.5-3b-instruct-delora-vicoqa`

Sau đó eval trên **test**:
- `eval_preds_{lora,tinylora,dora,delora}_test.json` (4 file)
- Bảng so sánh CoQA EM/F1 + `eval_compare_adapters_vicoqa_3b.json`

### Cách chạy (khuyến nghị CLI)
```bash
cd ViCoQA/qwen2.5-3b/LoRa
...
2. Config → Data → Profiling → Format → Train loop
3. Cell eval cuối (EVAL_SPLIT=test)
...
COMPARE_EVAL_PATH = "eval_compare_adapters_vicoqa_3b.json"
...
EVAL_SPLIT = "test"  # metric eval trên test → sinh eval_preds_*_test.json
